...
void get_data(CAIDA_Tuple data, uint64_t &id, uint64_t &timestamp) {
    memcpy(&timestamp, data.id+13, 8);
    id = BOBHash::BOBHash64((uint8_t*)&(data.id), 13, 42);
}
...
public:
    ...
    void Run(uint32_t memory, uint32_t KK) 
    {
        ...
        for (int i = 0; i < running_length; ++i) 
        {
            ...
            get_data(dataset[i], id, timestamp);
            // if (i%100000==0) std::cout << i << ' ' << id << ' ' << timestamp << std::endl;
            if (i%1000000==0) printf("i = %d\n",i);
            ...
            #ifdef TIME_BASED
            if (id_map[id] > 1 && timestamp > last_time[id]) 
            {
                correct_detector->insert(id, last_time[id], timestamp);
                //KLL_sketch->insert(id, last_time[id], timestamp);

                begin=clock();
                KLL_sketch->insert(id, timestamp - last_time[id]);
                finish=clock();
            ...
            if (id_map[id] > 1) 
            {
                ...
                //KLL_sketch->insert(id, last_time[id], i);
            }
            last_time[id] = i;
            #endif
        }
