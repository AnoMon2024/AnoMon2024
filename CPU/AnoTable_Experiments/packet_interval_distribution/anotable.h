...
public:
    ...
    const int cell_proportion[4] = {16, 4, 2, 1};

    int update_cnt;
    int insert_cnt;

    int cell_cnt[LINE_CNT];
    int *cells[LINE_CNT];
    ...
    static vector<pair<int, int>> get_subinterval(int l, int r) {
        ...
        // return get_subinterval_uniform(l, r);
    }
    
    CellSketch(int memory) {
        int sum_p = 0;
        for (int i = 0; i < 4; i++) {
            sum_p += cell_proportion[i];
        }
        int t = memory / sizeof(int) / sum_p;
        for (int i = 0; i < LINE_CNT; i++) {
            cell_cnt[i] = t * cell_proportion[i];
...
public:
    ...
    AnoTable(int memory) {
        int dleft_memory = memory / 2;
        int sketch_memory = memory / 2;
        ...
        if (RECORD_DELAY)
            cellsketch = new CellSketch(sketch_memory);
    }
    ...
    void insert(TUPLES flow_id, int arrive_time, int delay=-1) {
        for (int i = 0; i < D; i++) {
            ...
            if (counter[i][pos].ID == flow_id) {
                ...
                counter[i][pos].last_time = arrive_time;
                if (RECORD_DELAY)
                    cellsketch->insert(flow_id, delay);
                return;
            }
        }
    }
    ...
    vector<pair<int, int>> query_delay(TUPLES flow_id) {
        ...
        for (int i = 0; i < D; i++) {
            int pos = BOBHash(flow_id, i) % L;
            if (counter[i][pos].ID == flow_id) {
                return cellsketch->query(flow_id);
            }
        }
    ...
    void check_memory() {
        ...
        cout << "dleft: " << used << ' ' << D*L << ' ' << (double)used / D / L << endl;
    }

private:
    Counter **counter;
    int L;
    CellSketch *cellsketch;
};
