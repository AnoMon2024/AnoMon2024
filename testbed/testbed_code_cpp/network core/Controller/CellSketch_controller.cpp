...
{
  uint8_t class_flag;
  uint8_t read_bucket1;
  uint8_t read_bucket2;
  uint8_t read_cell_part1;
  uint8_t read_cell_part2;
  uint8_t read_bucket1_srciptag;
  uint8_t read_bucket1_dstiptag;
  uint8_t read_bucket2_srciptag;
  uint8_t read_bucket2_dstiptag;
  uint8_t read_bucket1_to_cell_part1;
  uint8_t read_bucket1_to_cell_part2;
  uint8_t read_bucket2_to_cell_part1;
  uint8_t read_bucket2_to_cell_part2; 
  uint16_t bgn_idx;
  uint16_t end_idx;
  //uint16_t zero_zone;
};
...
{
  //send packet for read registers
  for(uint16_t bgn_idx = 0; bgn_idx < array_size ; bgn_idx += bench_size) 
  {
    ...
    uint16_t cur_end_idx = htons(std::min(bgn_idx+bench_size,array_size - 1));

    pkt.eth.ethtype = 0xffff;
    pkt.info.bgn_idx = cur_bgn_idx;
    pkt.info.end_idx = cur_end_idx;
    pkt.info.class_flag = 1;
    pkt.info.read_bucket1 = 2;
    pkt.info.read_bucket2 = 2;
    pkt.info.read_cell_part1 = 2;
    pkt.info.read_cell_part2 = 2;
    pkt.info.read_bucket1_to_cell_part1 = 2;
    pkt.info.read_bucket1_to_cell_part2 = 2;
    pkt.info.read_bucket2_to_cell_part1 = 2;
    pkt.info.read_bucket2_to_cell_part2 = 2;
  ...
  for(uint16_t bgn_idx = 0; bgn_idx < array_size / 4 ; bgn_idx += bench_size) 
  {
      bf_pkt *bfpkt = NULL;
      collect_packet pkt{};
      while(1) 
      ...
      uint16_t cur_bgn_idx = htons(bgn_idx);
      uint16_t cur_end_idx = htons(std::min(bgn_idx + bench_size,array_size / 4 - 1));

      pkt.eth.ethtype = 0xffff;
      pkt.info.class_flag = 1;
      pkt.info.bgn_idx = cur_bgn_idx;
      pkt.info.end_idx = cur_end_idx;
      pkt.info.read_bucket1_srciptag = 2;
      pkt.info.read_bucket1_dstiptag = 2;
      pkt.info.read_bucket2_srciptag = 2;
      pkt.info.read_bucket2_dstiptag = 2;
      if(bf_pkt_data_copy(bfpkt, (uint8_t*)&pkt, collect_pkt_sz) != 0) {
          printf("Failed data copy\n");
      }
...
namespace bfrt
{
    namespace CellSketch
    {
        ...
        {   
            ...
            devMgr.bfRtInfoGet(dev_tgt.dev_id, "CellSketch", &bfrtInfo);
        }
...
int main(int argc, char **argv) {
    ...
    bfrt::CellSketch::init();

    //read eight times, for bucket size from 256 to 32768
    for(int i = 0; i < 8; ++i) 
    {
      ...
      std::cout << "sleep 5s, wait for rx packets, num: " << (256 << i) * 9 << std::endl;
      usleep(5000000);
      diff_ts = (end_tmv.tv_sec-bgn_tmv.tv_sec)*1000000+(end_tmv.tv_usec-bgn_tmv.tv_usec);
      printf("receive all rx packets spend time: %llu\n",diff_ts);
    }
    return status;
}
