...
template<typename T>
inline uint32_t BOBHash(const T& data, uint32_t seed = 0);
...
const int PRIME_CNT = 1229;

static const uint32_t prime[PRIME_CNT] = {
...
template<typename T>
inline uint32_t BOBHash(const T& data, uint32_t seed){
    return Hash::BOBHash32((uint8_t*)&data, sizeof(T), seed % PRIME_CNT);
}
