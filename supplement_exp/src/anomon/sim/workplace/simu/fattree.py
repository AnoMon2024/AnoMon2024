...
# ---- supplement_exp E3/E5 knobs (added for TON revision) ----
parser.add_argument('--n_flows',default=100000,type=int,help='total number of flows (flow-scale sweep, E3)')
parser.add_argument('--epoch',default=10,type=float,help='epoch length sec_time (E5/short-lived)')
parser.add_argument('--all_time',default=100,type=int,help='total simulated time')
parser.add_argument('--outcsv',default="",type=str,help='append a result row to this CSV')
parser.add_argument('--tag',default="",type=str,help='free-text tag written into the result row')
global args
...
# ---- supplement_exp result writer (E3/E5): append F1 row to outcsv ----
def _f1(p, r):
    return (2 * p * r / (p + r)) if (p + r) > 0 else 0.0
if args.outcsv:
    import os as _os
    loc_p = check_prec / check_num if check_num else 0.0
    loc_r = check_recall / check_num if check_num else 0.0
    afg_p = edge_prec / edge_num if edge_num else 0.0
    afg_r = edge_recall / edge_num if edge_num else 0.0
    header = "tag,algo,culprit_typ,fg_typ,n_flows,k,mem,epoch,all_time,err_flow_ratio,window,test,loc_prec,loc_recall,loc_f1,afg_prec,afg_recall,afg_f1\n"
    row = "%s,%s,%s,%s,%d,%d,%s,%s,%d,%s,%s,%d,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f\n" % (
        args.tag, args.algo, culprit_name[culprit_typ], fg_typ, n_flows, k, str(memcost),
        str(sec_time), all_time, str(args.err_flow_ratio), str(args.window), args.test,
        loc_p, loc_r, _f1(loc_p, loc_r), afg_p, afg_r, _f1(afg_p, afg_r))
    newfile = not _os.path.exists(args.outcsv)
    with open(args.outcsv, "a") as _fo:
        if newfile:
            _fo.write(header)
        _fo.write(row)
    print("RESULT_ROW:", row.strip())
