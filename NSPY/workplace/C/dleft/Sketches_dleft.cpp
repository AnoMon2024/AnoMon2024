...
# include <vector>
// #include "lala.h"
#include "../../../AnoTable/hotfilter/HotFilter.h"
#include "../../../AnoTable/sumax/sumax.h"
#include "../../../AnoTable/dleft/dleft.h"
#include "../../../AnoTable/marple/marple.h"
...
typedef HotFilter<10,13333> ESketch;
typedef DLeft<3,40000> DL;
typedef DLeft<3,13333> EDL;

typedef SuMaxSketch<60000,3> SM;
typedef SuMaxSketch<75000,3> ESM;

typedef MARBLE<60000> MB;
typedef MARBLE<75000> EMB;
...
public:
	...
	double ratio_large_interval(vector<ESketch>& sketch, KeyType type, TUPLES key){
		vector<uint32_t> res(INTERVAL_CNT, 0);
		uint32_t all=0;
		for(int i=0;i<e_num;i++){
			vector<uint32_t> tmp = sketch[i].query_partial_key(key, type, 0xffffff00);
	...
	int SSum(vector<ESketch>& sketch, KeyType type, TUPLES key){
		// vector<uint32_t> res(INTERVAL_CNT, 0);
		uint32_t all=0;
		for(int i=0;i<e_num;i++){
			vector<uint32_t> tmp = sketch[i].query_partial_key(key, type, 0xffffff00);
	...
	vector<int> ab_fg(int top,int typ){
		...
		for(int i=0;i<e_num;i++){
			for(int j=0;j<FATK/2;j++){
				...
				double ratio = 0;
				if(typ==0||typ==1)
					ratio = SSum(insketch,srcIP,edge_fg) - SSum(outsketch,srcIP,edge_fg);
				else
					ratio = ratio_large_interval(outsketch,srcIP,edge_fg);
	...
	double check_ratio_with_fid(uint32_t _srcIP, uint32_t _dstIP, uint16_t _srcPort, uint16_t _dstPort, uint8_t _proto , int typ) {
		TUPLES edge_fg=TUPLES(_srcIP,_dstIP,_srcPort,_dstPort,_proto);
		double ratio = 0;
		if(typ==0||typ==1)
			ratio = SSum(insketch,five_tuples,edge_fg) - SSum(outsketch,five_tuples,edge_fg);
		else
			ratio = ratio_large_interval(outsketch,five_tuples,edge_fg);
		return ratio;
	}
...
PYBIND11_MODULE(Sketches, m) {
    ...
    sketches.def(py::init<>())
		...
		.def("check_ratio_with_fid", &Sketches::check_ratio_with_fid)
        .def("update_core_key", &Sketches::update_core_key)
        .def("get_ab_fs", &Sketches::get_ab_fs)
		.def("get_extra_bd", &Sketches::get_extra_bd)
	...
	py::class_<ESketch>(sketches, "HotFilter")
		.def(py::init<>())
		...
		.def("query_pre", &ESketch::query_pre);
	...
	py::class_<DL>(sketches, "DL")
		...
		.def("query_arrive_pre",&DL::query_arrive_pre)
		.def("query_arrive",&DL::query_arrive)
        .def("query", &DL::query);
	...
	py::class_<EDL>(sketches, "EDL")
		...
		.def("query_arrive_pre",&EDL::query_arrive_pre)
		.def("query_arrive",&EDL::query_arrive)
        .def("query", &EDL::query);
