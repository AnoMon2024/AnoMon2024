                ...
                if self.rate<10001:
                    for key in self.culprit_time.keys():
                        if not (self.env.now>=key and self.env.now<self.culprit_time[key]):
                            continue
                        #print("creating delay--fg: ",packet.fg_id)
                        yield self.env.timeout(55/100000)
                ...
                if self.algo=="dleft":
                    if self.layer == 'edge':
                        if self.out.out.layer != 'leaf':
                            # if packet.fg_id == list(self.culprit_flows)[0] and packet.is_big:
                            #     ratio_before = self.sketch2.query_pre(src_ip,dst_ip,src_port,dst_port,_proto)
                            #     print("ratio_before:",ratio_before)
                        
                            interval = int((self.env.now-packet.time)*1000000)
                            # if packet.fg_id == list(self.culprit_flows)[0]:
                            #     print("interval:",interval,", fg_id:",packet.fg_id,", is_big:",packet.is_big,", packet_time:",packet.time,",now:",self.env.now,",last_interval:",packet.arrive_interval)
                            self.sketch2.insert_pre(src_ip,dst_ip,src_port,dst_port,_proto,interval) #up
                        
                            # if packet.fg_id == list(self.culprit_flows)[0] and packet.is_big:
                            #     print("--port insert in cul fg--  fg_id:",packet.fg_id,",flow_id:",packet.flow_id,",node id:",self.nod_id)
                            #     ratio_after = self.sketch2.query_pre(src_ip,dst_ip,src_port,dst_port,_proto)
                            #     print("ratio_after:",ratio_after,", src_ip:",src_ip,", dst_ip:",dst_ip,", src_port",src_port,", dst_port",dst_port,",proto",_proto,", interval:",interval)
                        ...
                        if self.out.out.layer == 'leaf':
                            if randint(1,100)==50:
                                ...
                                # if packet.fg_id == list(self.culprit_flows)[0]:
                                #     print("interval:",interval,", fg_id:",packet.fg_id,", is_big:",packet.is_big,", packet_time:",packet.time,",now:",self.env.now,",last_interval:",packet.arrive_interval)
                                self.sketch3.insert_pre(src_ip,dst_ip,src_port,dst_port,_proto,interval) #down
                            # if packet.fg_id == list(self.culprit_flows)[0] and packet.is_big:
                            #     print("--port insert out cul fg--  fg_id:",packet.fg_id,",flow_id:",packet.flow_id,",node id:",self.nod_id)
