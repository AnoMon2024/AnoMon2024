            ...
            if self.zero_downstream_buffer:
                ...
                if self.use_netseer == 0:
                    ...
                    if self.layer == 'core' or self.layer == 'aggregation' or self.layer == 'edge':
                        if self.algo == "dleft":
                            if packet.fg_id in self.ab_fgroup:
                                # tim = int(self.env.now*1000000)
                                tim=0
                                if self.algo=="dleft":
                                    tim = int(packet.arrive_time*1000000)
                                elif self.algo=="sumax" or self.algo=="marple":
                                    tim = int(packet.arrive_interval*1000000)
                                # print("jili")
                                wait = int((self.env.now-packet.arrive_time)*1000000)
                                if self.layer == 'core' and packet.is_big == 1:
                                    last_arrive = self.sketch1.query_arrive_pre(src_ip,dst_ip,src_port,dst_port,_proto)
                                    #print("flow_id:",packet.flow_id,",fg_id:",packet.fg_id,", last_arrive:",last_arrive)
                                ...
                                # if self.layer == 'core' and packet.is_big == 1:
                                #     m_in = self.sketch1.query_interval_pre(src_ip,dst_ip,src_port,dst_port,_proto)
                                #     print("flow_id:",packet.flow_id,",fg_id:",packet.fg_id,", tim:",tim,", max_interval:",m_in)
                        else:
                                tim=0
                                if self.algo=="dleft":
                                    tim = int(packet.arrive_time*1000000)
