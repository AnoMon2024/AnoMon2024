...
class FIBDemux:
    ...
    def __init__(self,
        ...
        self.pd_dict = packet_drop_num
        self.err_rate = 1
        self.netseer = netseer
        if ends:
            self.ends = ends
        else:
            self.ends = dict()
        ...
        if flow_id in self.ends:
            ...
            try:
                if self.culprit_typ==0 or self.culprit_typ==1:
                    ...
                    if self.flag == 0 or x<(10000 - 10000*self.err_rate):
                        if self.flag == 1 and self.culprit_typ == 1 and (packet.fg_id in self.culprit_flows) and (self.nod_id in self.culprit_switches[packet.fg_id]) and packet.is_big == 1:
                            if packet.flow_id in self.fib.keys():
                        ...
                        if self.culprit_typ==0:
                            if (packet.fg_id in self.culprit_flows) and (self.nod_id in self.culprit_switches[packet.fg_id]) and packet.is_big == 1 :    
                                self.netseer.insert(packet)
                                # if packet.flow_id in self.pd_dict.keys():
                                #     if self.pd_dict[packet.flow_id]>0 :
                                #         self.pd_dict[packet.flow_id] = self.pd_dict[packet.flow_id] - 1
                                #     else:
                                #         self.outs[self.fib[packet.flow_id]].put(packet)
                            else:
                                self.outs[self.fib[packet.flow_id]].put(packet)
                        ...
                        ######################################################loop####################################################################
                            ...
                            if not ((packet.fg_id in self.culprit_flows) and (self.nod_id in self.culprit_switches[packet.fg_id]) and packet.is_big == 1):    
                                self.outs[self.fib[packet.flow_id]].put(packet)
                            else:
