#ifndef _Hot_Filter_H_
#define _Hot_Filter_H_

#include "HeavyPart.h"
#include "../coldsampler/coldsampler.h"
...
public:
    ...
    HotFilter() {
        ...
        for (int i = 0; i < INTERVAL_CNT; i++) {
            light_part[i] = new ColdSampler(light_mem);
        }
    }
    ~HotFilter(){
        for (int i = 0; i < INTERVAL_CNT; i++) {
    ...
    uint32_t query_pre(uint32_t src_ip,uint32_t dst_ip,uint16_t src_port,uint16_t dst_port,uint8_t _proto){
        uint32_t all=0;
        TUPLES item(src_ip,dst_ip,src_port,dst_port,_proto);
        vector<uint32_t> tmp = query(item);
    ...
    vector<uint32_t> query_partial_key(TUPLES key, KeyType type, uint32_t mask=0xffffffff) {
        ...
        for (auto it = mp.begin(); it != mp.end(); it++) {
            if (
                ...
                type == allID           && key.srcIP() == it->first.srcIP() && key.srcPort() == it->first.srcPort() && key.dstIP() == it->first.dstIP() && key.dstPort() == it->first.dstPort() && key.proto() == it->first.proto()
            ) {
                for (int i = 0; i < INTERVAL_CNT; i++) {
                    ans[i] += it->second[i];
                }
